...
public class CustomVideoView extends RelativeLayout implements View.OnClickListener,
    ...
    private static final int TIME_INVAL = 1000;//每隔一秒发送一个Message
    private static final int STATE_ERROR = -1;
    private static final int STATE_IDLE = 0;
    private static final int STATE_PLAYING = 1;
    private static final int STATE_PAUSING = 2;
    private static final int LOAD_TOTAL_COUNT = 3;//错误重试次数
    /**
     * UI
     */
    ...
    /**
     * when the video paused not via auto-pause
     * @param isRealPause
     */
    ...
    //入口方法 先load
    public void load() {
        if (this.playerState != STATE_IDLE) {//不处于空闲状态直接返回
            return;
        }
        Log.d(TAG, "do play url = " + this.mUrl);
        showLoadingView();//anim
        try {
            setCurrentPlayState(STATE_IDLE);
            checkMediaPlayer();//check mediaplayer null
            mute(false);
    ...
    /**
     * when the surface is available we can load() the MediaPlayer
     */
    @Override
    ...
    public interface ADVideoPlayerListener {
        ...
        public void onBufferUpdate(int time);//视频播放到第几秒

        public void onClickFullScreenBtn();//跳转全屏
